...
## Project
Randstad scraper for peviitor.ro (Node.js, ESM, Jest)

## About This Repo
This is the **Randstad Romania scraper** derived from the EPAM template. It uses HTML scraping (cheerio) for randstad.ro and jobRapid.ro, plus ANOFM API by CIF.
...
- **The HTML scraping logic in `index.js** (`searchRandstad`, `searchJobRapid`) is Randstad-specific. The output shape (`mapToJobModel`, `transformJobsForSOLR`) stays uniform across all scrapers.
...
# E2E tests (real randstad.ro + jobRapid.ro, SOLR conditional)
npm run test:e2e
...
- `tests/validate-randstad-jobs.js` — CI fast validator (HEAD only); thin wrapper over src/job-validator.js + solr.js
