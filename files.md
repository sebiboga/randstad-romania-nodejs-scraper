...
| `tests/company.json` | Mock ANAF company data for Randstad used in unit tests |
| `tests/validate-randstad-jobs.js` | **Randstad-specific fast validator (used by CI).** HEAD requests only, reads CIF from config. Called nightly by `automation-testing.yml`. Supports `--dry-run` and `--delete`. |
...
| `tests/e2e/scraper.test.js` | E2E tests - full pipeline with real APIs (randstad.ro, jobRapid.ro, ANAF, SOLR) |
...
| `AI-DERIVATION-GUIDE.md` | ~~Comprehensive playbook for AI agents deriving a new scraper~~ *(deleted — belongs only in the EPAM template, not in derived repos)* |
...
| `ROBOTS.md` | robots.txt analysis and scraping policy for randstad.ro and jobRapid.ro |
...
| `delete_request.json` | **Manual maintenance tool** — SOLR payload to delete ALL jobs for CIF 17549799. Use only when you need to wipe Randstad jobs from SOLR entirely. Run with: `curl --user "${SOLR_AUTH}" "https://solr.peviitor.ro/solr/job/update?commit=true" -H "Content-Type: application/json" -d @delete_request.json` |
...
- Full workflow: validate company (ANAF+Peviitor) → scrape portals (randstad.ro + jobRapid.ro + ANOFM) → transform → upsert SOLR → generate docs/jobs.md